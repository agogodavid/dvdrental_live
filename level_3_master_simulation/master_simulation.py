...
workspace_root = os.path.dirname(script_dir)  # Go up from level_3_master_simulation
level_1_basic = os.path.join(workspace_root, 'level_1_basic')
film_system = os.path.join(script_dir, 'film_system')  # Add film_system subdirectory
if film_system not in sys.path:
    sys.path.insert(0, film_system)
...
class SimulationConfig:
    ...
    # Film Releases - now loaded from config.json
...
def load_config(config_file='config.json', override_database=None) -> dict:
    ...
    Args:
        config_file: Config file name (will search in multiple locations)
    ...
    if override_database:
        ...
        logger.info(f"Using database: {override_database} (overriding config)")
...
def add_film_batch(mysql_config: dict, num_films: int, category_focus: str = None, description: str = "", sim_date: date = None, add_inventory: bool = True) -> int:
    """Add new films to the database with optional inventory copies using the new film generator
    ...
    Args:
        ...
        sim_date: Simulation date for release
        add_inventory: Whether to add inventory copies (True) or just record to film_releases (False)
    ...
    try:
        ...
        films_added = film_generator.add_film_batch(
            num_films, category_focus, description, sim_date, add_inventory=add_inventory
        )
...
def get_hot_category_for_week(week_num: int, hot_categories: List[Dict]) -> Tuple[str, float]:
    """Get the hot category and inventory boost for a given week
    
    Returns: (category_name, inventory_boost_multiplier)
    """
    for hot_cat in hot_categories:
        if week_num in hot_cat.get('weeks', []):
            return hot_cat['category'], hot_cat.get('inventory_boost', 1.0)
    # Default to balanced strategy when no hot category
    return None, 1.0


def get_weekly_film_release_count(week_num: int, strategy: Dict) -> Tuple[int, str]:
    """Calculate number of films to release this week based on strategy
    
    Returns: (count, description)
    """
    weekly_rate = strategy.get('weekly_releases', 0.5)
    hot_cat, _ = get_hot_category_for_week(week_num, strategy.get('hot_categories', []))
    
    # Base count from weekly rate
    base_count = max(0, int(weekly_rate * 4))  # ~2-4 films per cycle at 0.5 rate
    
    # Extra films during hot category launches
    if hot_cat and (week_num % strategy.get('hot_category_rotation_weeks', 12)) == 0:
        extra_count = 2
        desc = f"Hot category launch: {hot_cat}"
        return base_count + extra_count, desc
    
    desc = "Steady release"
    return base_count, desc


def get_film_releases_for_week(week_num: int) -> Tuple[bool, int, str, str]:
    """Check if there's a film release this week (legacy interface)"""
    # This now uses the strategy from config
    if not hasattr(SimulationConfig, '_film_strategy'):
        return False, 0, None, ""
    
    count, desc = get_weekly_film_release_count(week_num, SimulationConfig._film_strategy)
    hot_cat, _ = get_hot_category_for_week(week_num, SimulationConfig._film_strategy.get('hot_categories', []))
    
    if count > 0:
        return True, count, hot_cat, desc
    return False, 0, None, ""


def add_incremental_weeks(mysql_config: dict, config: dict, num_weeks: int, seasonal_drift: float = 0.0, current_sim_week: int = 0, total_weeks: int = 0) -> int:
    """
...
def main():
    ...
    print("\n")
    logger.info("╔" + "═" * 78 + "╗")
    logger.info("║" + " MASTER DVD RENTAL SIMULATION ".center(78) + "║")
    logger.info("╚" + "═" * 78 + "╝")
    
    # Parse command-line arguments
    parser = argparse.ArgumentParser(description='Master DVD Rental Simulation')
    parser.add_argument('--config', type=str, default='config.json', help='Config file to use (default: config.json)')
    ...
    config = load_config(config_file=args.config, override_database=args.database)
    ...
    # Load film release strategy from config
    film_strategy = master_config.get('film_release_strategy', {
        'weekly_releases': 0.5,
        'hot_category_rotation_weeks': 12,
        'hot_categories': []
    })
    SimulationConfig._film_strategy = film_strategy
    
    # Build display of hot categories (legacy FILM_RELEASES for display purposes)
    SimulationConfig.FILM_RELEASES = []
    hot_categories = film_strategy.get('hot_categories', [])
    for hot_cat in hot_categories:
        for week in hot_cat.get('weeks', []):
            if week <= SimulationConfig.TOTAL_WEEKS:
    ...
    logger.debug(f"Loaded film release strategy: {len(hot_categories)} hot categories for {SimulationConfig.TOTAL_WEEKS}-week simulation")
    
    # Handle --season argument to enable/disable seasonal variations
    enable_seasons = True  # Default enabled
    if args.season:
        enable_seasons = args.season.lower() in ['yes', 'true', '1', 'on']
        if not enable_seasons:
            logger.info("Seasonal variations DISABLED via --season flag")
    ...
    try:
        ...
        while weeks_added < remaining_weeks:
            ...
            # MARKET RELEASES: Add all films available on the market (no inventory yet)
            market_releases = SimulationConfig._film_strategy.get('market_weekly_releases', 8)
            if market_releases > 0:
                current_date = SimulationConfig.START_DATE + timedelta(weeks=current_sim_week)
                market_desc = f"Market releases: {market_releases} films available"
                # Add to film_releases table but NOT to inventory yet
                add_film_batch(mysql_config, market_releases, category_focus=None, 
                              description=market_desc, sim_date=current_date, add_inventory=False)
            
            # HOT CATEGORY PURCHASES: Selective purchasing from hot categories
            ...
            if has_films and num_films > 0:
                ...
                # Add films to inventory (representing purchasing decision for hot category)
                add_film_batch(mysql_config, num_films, category, film_desc, sim_date=current_date, add_inventory=True)
            ...
            if current_sim_week % 10 == 0 and current_sim_week > 0:
                try:
                    conn = mysql.connector.connect(
                        host=mysql_config['host'],
                        user=mysql_config['user'],
                    ...
                    cursor.close()
                    conn.close()
                    logger.info(f"   📊 Week {current_sim_week}: {inventory_count} inventory items, {film_count} films")
                except Exception as e:
                    logger.warning(f"Could not get inventory/film counts: {e}")
            ...
            # Use --season argument if provided, otherwise calculate from date
            if args.season is not None:
                seasonal_drift = args.season
            else:
            ...
            added_weeks = add_incremental_weeks(mysql_config, config, weeks_to_add, seasonal_drift, current_sim_week, SimulationConfig.TOTAL_WEEKS)
        ...
        logger.info(f"✓ Inventory Growth: {inventory_growth:.1f}% (from {initial_inventory:,} to {total_inventory:,})")
